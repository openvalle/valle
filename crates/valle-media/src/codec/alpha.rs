...
use crate::codec::ffi::{ffmpeg_init, fill_plane0, write_mp4_header};
...
impl TransparentVideoMuxer {
    ...
    fn open_with_clock(
        ...
        let codec = encoder::find_by_name(QTRLE_ENCODER)
            .ok_or_else(|| anyhow!("transparent encoder '{QTRLE_ENCODER}' unavailable"))?;
...
mod tests {
    ...
    #[test]
    fn cfr_writer_keeps_every_lossless_rgba_frame() {
    ...
    #[test]
    #[cfg(feature = "tool-segment")]
    fn timestamped_writer_keeps_a_single_nonzero_pts_frame() {
        let root = tempfile::tempdir().unwrap();
        let path = root.path().join("foreground.mov");
        let frame = RgbaFrame {
            width: 2,
