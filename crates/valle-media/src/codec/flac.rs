...
use crate::{ffi::ffmpeg_init, frame::AudioBuffer};
...
/// Human- and machine-reportable description of the only f32-to-FLAC conversion policy.
...
impl FlacPcm24Writer {
    ...
    pub fn create(path: &Path, sample_rate: u32, channels: u16) -> Result<Self> {
        ...
        let flac_codec = encoder::find(codec::Id::FLAC)
            .ok_or_else(|| anyhow!("the linked libav build has no FLAC encoder"))?;
...
mod tests {
    ...
    #[test]
    fn streaming_flac_roundtrip_preserves_length_and_pcm24_precision() {
    ...
    #[test]
    fn strict_validation_rejects_truncated_and_trailing_data() {
        let root = tempfile::tempdir().unwrap();
