//! Bounded video sequencing and exact clocks, independent of the selected native ABI.
use crate::codec::backend::{self, Inner, Version, backend_type};
use crate::{codec::TimeBase as Rational, frame::RgbaFrame};
#[cfg(any(
...
pub(crate) struct DecodedVideoTiming {
    pub frame_count: u64,
...
#[derive(Debug)]
pub(crate) struct TimedVideoFrame<T> {
...
backend_type!(SequentialRgbaDecoder, video_sequence);
impl SequentialRgbaDecoder {
    pub fn open(path: &Path, threads: usize) -> Result<Self> {
        match backend::version()? {
            Version::V7 => backend::v7::video_sequence::SequentialRgbaDecoder::open(path, threads)
                .map(Into::into),
            Version::V8 => backend::v8::video_sequence::SequentialRgbaDecoder::open(path, threads)
                .map(Into::into),
            Version::V9 => backend::v9::video_sequence::SequentialRgbaDecoder::open(path, threads)
                .map(Into::into),
        }
    }
    pub fn metadata(&self) -> VideoMetadata {
        match &self.0 {
            Inner::V7(inner) => inner.metadata(),
            Inner::V8(inner) => inner.metadata(),
            Inner::V9(inner) => inner.metadata(),
        }
    }
    pub fn next_frame(&mut self) -> Result<Option<TimedVideoFrame<RgbaFrame>>> {
        match &mut self.0 {
            Inner::V7(inner) => inner.next_frame(),
            Inner::V8(inner) => inner.next_frame(),
            Inner::V9(inner) => inner.next_frame(),
        }
    }
}
#[cfg(any(feature = "tool-inpaint", feature = "tool-segment"))]
backend_type!(SequentialGray8Decoder, video_sequence);
#[cfg(any(feature = "tool-inpaint", feature = "tool-segment"))]
impl SequentialGray8Decoder {
    pub fn open(path: &Path, threads: usize) -> Result<Self> {
        match backend::version()? {
            Version::V7 => backend::v7::video_sequence::SequentialGray8Decoder::open(path, threads)
                .map(Into::into),
            Version::V8 => backend::v8::video_sequence::SequentialGray8Decoder::open(path, threads)
                .map(Into::into),
            Version::V9 => backend::v9::video_sequence::SequentialGray8Decoder::open(path, threads)
                .map(Into::into),
        }
    }
    pub fn metadata(&self) -> VideoMetadata {
...
pub(crate) fn decode_video_timing(path: &Path, threads: usize) -> Result<DecodedVideoTiming> {
    match backend::version()? {
        Version::V7 => backend::v7::video_sequence::decode_video_timing(path, threads),
        Version::V8 => backend::v8::video_sequence::decode_video_timing(path, threads),
        Version::V9 => backend::v9::video_sequence::decode_video_timing(path, threads),
    }
}
#[derive(Debug)]
#[cfg(feature = "tool-inpaint")]
pub(crate) struct MatchedVideoFrame {
...
#[cfg(feature = "tool-inpaint")]
pub(crate) struct MatchedVideoReader {
...
impl MatchedVideoReader {
    ...
    pub fn source_metadata(&self) -> VideoMetadata {
        self.source.metadata()
    }

    pub fn mask_metadata(&self) -> VideoMetadata {
        self.mask.metadata()
    }
...
pub(crate) struct VideoAudioBridge {
...
fn audio_stream_start_seconds(path: &Path) -> Result<Option<f64>> {
    match backend::version()? {
        Version::V7 => backend::v7::video_sequence::audio_stream_start_seconds(path),
        Version::V8 => backend::v8::video_sequence::audio_stream_start_seconds(path),
        Version::V9 => backend::v9::video_sequence::audio_stream_start_seconds(path),
    }
}
...
pub(crate) fn rescale_timestamp_nearest(
    value: i64,
    source_time_base: Rational,
...
pub(crate) fn rotate_rgba(source: RgbaFrame, degrees: u32) -> RgbaFrame {
...
#[cfg(any(feature = "tool-inpaint", feature = "tool-segment"))]
pub(crate) fn rotate_gray8(source: Gray8Frame, degrees: u32) -> Gray8Frame {
    if degrees == 0 || source.width == 0 || source.height == 0 {
        return source;
    }
    let (width, height) = (source.width, source.height);
