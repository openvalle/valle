...
/// The software path configures x264-specific CRF/preset controls; do not select a different
/// encoder merely because it also advertises H.264.
pub(crate) fn software_h264_encoder() -> Result<ff::Codec> {
    ffmpeg_init()?;
    encoder::find_by_name("libx264").ok_or_else(|| anyhow!(
        "the installed FFmpeg has no libx264 encoder required for software H.264 export; inspect it with `valle media capabilities` or explicitly select a supported hardware encoder"
    ))
}
...
impl Mp4Encoder {
    ...
    pub fn new(
        ...
        let codec = software_h264_encoder()?;
