...
use super::encode::{DEFAULT_X264_CRF, DEFAULT_X264_PRESET, software_h264_encoder};
use super::ffi::{
...
use super::shared_frame::{SharedVideoFrame, SharedVideoFramePool, VideoFrameTransport};
...
impl Muxer {
    ...
    fn open_configured(
        ...
        let audio = if include_audio {
            let acodec =
                encoder::find(codec::Id::AAC).ok_or_else(|| anyhow!("the installed FFmpeg has no AAC encoder; inspect it with `valle media capabilities`"))?;
