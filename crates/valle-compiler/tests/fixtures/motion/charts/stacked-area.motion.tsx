...
const X = scalePoint({ range: [150, 980], count: LABELS.length });
const Y = scaleLinear({ domain: DOMAIN, range: [590, 160] });
const ZERO = curve(
...
function LegendItem(ctx, { index, label, color }) {
  return (
    <View key="item" className="flex items-center" style={{ marginTop: index === 0 ? 0 : 18, width: 180, height: 32 }}>
      <View key="swatch" style={{ width: 16, height: 16, borderRadius: 4, backgroundColor: color }} />
      <Text key="name" style={{ marginLeft: 12, fontSize: 20, color: "#cbd5e1" }}>{label}</Text>
    </View>
  );
}
...
export default function StackedArea(ctx) {
  ...
  return (
    <Scene key="scene" className="relative h-full w-full" style={{ backgroundColor: "#07111a" }}>
      <Text key="eyebrow" className="absolute" style={{ left: 72, top: 40, fontSize: 18, letterSpacing: 3, color: "#22d3ee" }}>
        STACK / SHARED EDGE
      </Text>
      <Text key="title" className="absolute" style={{ left: 72, top: 72, fontSize: 40, color: "#e2e8f0" }}>
      ...
      {ticks(DOMAIN[0], DOMAIN[1], 4).map((value, index) => (
        <View
          key={`grid-${index}`}
          className="absolute"
          style={{ left: 150, top: Y.map(value), width: 830, height: 1, backgroundColor: "#1e293b" }}
        />
      ))}
      ...
      <Path key="mid-line" d={MID} fill="none" stroke="#67e8f9" strokeWidth="3" trimEnd={reveal} />
      <Path key="top-line" d={TOP} fill="none" stroke="#a5f3fc" strokeWidth="3" trimEnd={reveal} />
      {LABELS.map((label, index) => (
        <Text
          key={`lab-${index}`}
          className="absolute"
          style={{ left: X.at(index) - 24, top: 612, width: 48, fontSize: 16, color: "#94a3b8", textAlign: "center" }}
        >
          {label}
        </Text>
      ))}
      <View key="legend" className="absolute" style={{ left: 1040, top: 200, width: 180, height: 120 }}>
