export function ChartPanel(ctx, { rows, x, y, width, height }) {
  ...
  const { panel, accent, ink, hairline } = theme.colors;
  ...
  return (
    <View key="panel" className="absolute" style={{ borderStyle: "solid", left: x, top: y, width, height, borderRadius: theme.radius.panel, backgroundColor: panel, borderWidth: 1, borderColor: hairline }}>
      <Text key="kicker" className="absolute" style={{ left: 30, top: 26, fontSize: 15, letterSpacing: 3, color: accent }}>CHART / PREPARED SCALE</Text>
      {rows.map((row, index) => {
