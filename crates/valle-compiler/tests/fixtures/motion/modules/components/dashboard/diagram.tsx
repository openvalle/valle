  ...
  return (
    <View key="panel" className="absolute" style={{ borderStyle: "solid", left: x, top: y, width, height, borderRadius: theme.radius.panel, backgroundColor: theme.colors.panel, borderWidth: 1, borderColor: theme.colors.hairline }}>
      ...
      {edges.map((edge) => {
        ...
        return <Path key={edge.id} d={line([point(58 + from[0], 116 + from[1]), point(58 + to[0], 116 + to[1])])} fill="none" stroke={theme.colors.accent} strokeWidth="3" trimEnd={ctx.progress} style={{ opacity: 0.7 }} />;
      })}
      {nodes.map((node, index) => {
        const center = layout.centers[index];
        return <View key={node.id} className="absolute" style={{ borderStyle: "solid", left: 58 + center[0] - 78, top: 116 + center[1] - 35, width: 156, height: 70, borderRadius: theme.radius.node, backgroundColor: theme.colors.panelRaised, borderWidth: 2, borderColor: theme.colors.accent, opacity: 0.35 + ctx.progress * 0.65 }}><Text key={`label-${node.id}`} className="absolute" style={{ left: 8, top: 23, width: 140, fontSize: 17, color: theme.colors.ink, textAlign: "center" }}>{node.label}</Text></View>;
      })}
    </View>
  );
}
